...
local commit = shared.WIND_UI_CUSTOM_COMMIT or "99445a5dd2bcb48c06dbe53b251522739a7ea2ff"
	...
	timedFunction(
		function()
			return shared.WindUIDevMode
				...
				or game:HttpGet(
					"https://files.vapevoidware.xyz/VapeVoidware/WindUI/" .. tostring(commit) .. "/dist/main.lua",
					true
				)
		end,
		5,
	...
	if shared.VoidwareCustom then
		...
		elseif Tabs_Meta.updatefocused ~= nil and table.find(Tabs_Meta.updatefocused, string.lower(searchIndex)) then
			WindUIAdapter._updatefocusedtab = WindUIAdapter._updatefocusedtab
				...
				or section:Tab({ Title = "Update Focused", Icon = "life-buoy" })
		...
		elseif table.find(Tabs_Meta.misc, string.lower(searchIndex)) then
			...
			return result
		elseif Tabs_Meta.upgrades ~= nil and table.find(Tabs_Meta.upgrades, string.lower(searchIndex)) then
			WindUIAdapter._upgradestab = WindUIAdapter._upgradestab
				or section:Tab({ Title = "Upgrades", Icon = "arrow-up-circle" })
			local sec = WindUIAdapter._upgradestab:Section({
				Title = title,
				TextXAlignment = "Left",
				Icon = icon,
				TextSize = 17,
			...
			return result
		elseif Tabs_Meta.farm ~= nil and table.find(Tabs_Meta.farm, string.lower(searchIndex)) then
			WindUIAdapter._farmtab = WindUIAdapter._farmtab or section:Tab({ Title = "Farm", Icon = "sprout" })
			local sec = WindUIAdapter._farmtab:Section({
				Title = title,
				TextXAlignment = "Left",
				Icon = icon,
				TextSize = 17,
