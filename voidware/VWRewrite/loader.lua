...
local function http_function(url)
	if url == nil then
		return
	end
	url = tostring(url)
	...
	if not (suc ~= nil and suc == true and res ~= nil and type(res) == "table") then
		return
	end
	if res[1] ~= nil and res[1] == game then
		return res[4]
	else
		return res[1]
	end
...
getgenv().pload = function(name, id, found)
	...
	if not suc and found then
		...
		if not (id ~= nil and tostring(id) == "loader") then
