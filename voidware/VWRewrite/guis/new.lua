...
local N=(M.X/A.Scale)+K.X
local O=(M.Y/A.Scale)+K.Y

if d.isMobile then
local P=workspace.CurrentCamera.ViewportSize/A.Scale
local Q=H.AbsoluteSize.X/A.Scale
local R=H.AbsoluteSize.Y/A.Scale
N=math.clamp(N,0,math.max(0,P.X-Q))
O=math.clamp(O,0,math.max(0,P.Y-R))
end
H.Position=UDim2.fromOffset(N,O)
end
end)
...
M.ScrollBarImageTransparency=d.isMobile and 0.4 or 0.5
M.ScrollBarThickness=d.isMobile and 8 or 4
...
if N.UserInputType==Enum.UserInputType.Touch then
pcall(function()
ab.Parent.ScrollingEnabled=false
end)
end
end)
M.InputEnded:Connect(function(N)
if N.UserInputType==Enum.UserInputType.Touch then
pcall(function()
ab.Parent.ScrollingEnabled=true
end)
...
local R=d.isMobile and 40 or 26
local S=O and 1 or 0
for T,U in aa.List do
local V=Instance.new"TextButton"
V.Name=U.."Option"
V.Size=UDim2.new(1,0,0,R)
V.Position=UDim2.fromOffset(0,S*R)
V.BackgroundColor3=o.Main
V.BorderSizePixel=0
V.AutoButtonColor=false
V.Text="   "..U
V.TextColor3=o.Text
V.TextXAlignment=Enum.TextXAlignment.Left
V.TextSize=d.isMobile and 14 or 13
V.FontFace=o.Font
V.Parent=M
addTooltip(V,U)
...
for W,X in Q do
local Y=X.Button
local Z=X.Name:lower()
local _=(T==""or Z:find(T))
...
Y.Position=UDim2.fromOffset(0,(U-1)*(d.isMobile and 40 or 26))

Y.TextTransparency=0
Y.BackgroundTransparency=0
else
Y.TextTransparency=0.6
...
J.Size=UDim2.new(1,0,0,Y+Z)

M.Position=UDim2.fromOffset(0,Y)
M.Size=UDim2.new(1,0,0,Z)
end
...
Toggled=c(
`{tostring(an.Name)}_{tostring(ac.Name)}_{tostring(an.SavingID)}_{tostring(
an.ExtraText
)}`
),
}
...
az.BackgroundColor3=ao.StarActive and Color3.fromRGB(255,255,127)
...
ao.OptionsVisibilityChanged=a.createCustomSignal(
...
n
:Tween(N,TweenInfo.new(0.25,Enum.EasingStyle.Quad),{
BackgroundTransparency=1,
}).Completed
:Once(function()
pcall(function()
...
CategoryApi=ac,
ExpandEvent=c(
`ModuleCategory_ExpandEvent_{tostring(an.Name)}_{tostring(ac.Name)}`
),
...
ay.VerticalAlignment=ao.UpExpand and Enum.VerticalAlignment.Bottom
...
d.http_function(
`https://code.sarris.dev/voidware/translations/main/LanguageFlags.json`,
true
)
)
end,
...
return game:GetService"HttpService":JSONDecode(
d.http_function(
`https://code.sarris.dev/voidware/translations/main/Languages.json`,
true
)
)
end,
...
return decode(
d.http_function(
`https://code.sarris.dev/voidware/translations/main/locales/{ab}.json`,
true
)
)
end,
...
ah.Position.X.Offset,
ah.Position.Y.Offset-(av-41)
),
})
else
n:Tween(ah,TweenInfo.new(0.2,Enum.EasingStyle.Quad),{
...
loadstring(
d.http_function(
"https://code.sarris.dev/voidware/VWRewrite/"
...
aj.BackgroundTransparency=1
aj.Image=u"vape/assets/new/search.png"
...
ak.BackgroundTransparency=1
ak.Image=u"vape/assets/new/legit.png"
ak.Parent=ah
...
aD
and aE
and aD:FindFirstChild"Dots"
and aE:FindFirstChild"Dots"
then
...
aq.BackgroundTransparency=1
aq.Image=u"vape/assets/new/vape.png"
aq.Parent=ao
...
loadstring(
d.http_function(
"https://code.sarris.dev/voidware"
.."/VWRewrite/"
..readfile"vape/profiles/commit.txt"
.."/loader.lua",
true
